...
def parse_args():
    ...
    parser.add_argument('--best_iou', default=None, type=float)
...
def up_iou(iou, config):
    with open(os.path.join('./models/%s_%s_woDS' % (config['dataset'], config['arch']), 'config.yml')) as f:
        doc = yaml.load(f)
    doc['best_iou'] = iou
    with open(os.path.join('./models/%s_%s_woDS' % (config['dataset'], config['arch']), 'config.yml'), 'w') as f:
        yaml.dump(doc, f)
...
def main():
    ...
    # with open('models/%s/config.yml' % config['name']) as f:
    #     doc = yaml.load(f)
    # iou = doc['best_iou']
    if os.path.exists('models/%s/config.yml' % config['name']):
        with open('models/%s/config.yml' % config['name'], 'r') as f:
            # config['best_iou'] = iou
            doc = yaml.load(f)
            config['best_iou'] = doc['best_iou']
            f.close()
    ...
    model = model.cuda()
    archs.init_weights(model, init_type=str(config['init_type']), gain=0.02)
    ...
    if config['best_iou'] == None:
        best_iou = 0
    else:
        best_iou = float(config['best_iou'])
    trigger = 0
    for epoch in range(config['epochs']):
        ...
        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), 'models/%s/model.pth' % config['name'])
            up_iou(float(val_log['iou']), config)
